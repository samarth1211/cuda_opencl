...
// cuda kernel
__global__ void rt_noConst_kernel(struct Sphere *s, cudaSurfaceObject_t target, dim3 texDim,float time)
{
	...
	{
		uchar4 data = make_uchar4(r*255,g*255,b*255, 255);
		surf2Dwrite(data, target, x * sizeof(uchar4), y, cudaBoundaryModeTrap);
	}
	
}

struct Sphere *sph = NULL;
...
int Initialize(void)
{
	...
	cuErr = cudaMalloc((void**)&sph,sizeof(struct Sphere) * SPHERES);
	if (cuErr!=cudaSuccess)
	{
	...
	cuErr = cudaMemcpy(sph,temp_Sph,sizeof(struct Sphere)*SPHERES,cudaMemcpyHostToDevice);
	if (cuErr!=cudaSuccess)
	{
...
int Update(void)
{
	...
	rt_noConst_kernel<<<block, thread >>>(sph,writeSurface, dim3(DIM_W, DIM_H),1.0f);
