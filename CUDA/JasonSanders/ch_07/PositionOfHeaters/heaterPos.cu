...
int Update(void)
{
	...
	float *temp_inSrc,*temp_constSrc,*temp_outSrc;
	temp_inSrc = dev_inSrc;
	temp_constSrc = dev_constSrc;
	temp_outSrc = dev_outSrc;
	for(int i=0;i<90;i++)
	{
		copy_const_kernel<<<blocks,threads>>>(temp_inSrc,temp_constSrc);
		blend_kernel<<<blocks,threads>>>(temp_outSrc,temp_inSrc);
		swapFunc(temp_inSrc,temp_outSrc);
	}
	...
	float_to_color<<<blocks,threads>>>(writeSurface,dev_constSrc,dim3(DIM,DIM));
