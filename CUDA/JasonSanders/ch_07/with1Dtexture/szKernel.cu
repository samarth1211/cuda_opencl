...
__global__ void blend_kernel(float *dst,bool dstOut)
{
	...
	int top = offset - DIM_H;
	int bottom = offset + DIM_H;
	if(y==0) top += DIM_H;
	if(y==DIM-1) bottom -= DIM_H;
...
int Initialize(void)
{
	...
	glTexImage2D(GL_TEXTURE_2D,0,GL_RGBA, DIM_W, DIM_H,0,GL_RGBA,GL_UNSIGNED_INT,NULL);
	...
	cuErr = cudaMalloc((void**)&dev_inSrc,4*DIM_W*DIM_H);
	if(cuErr!=cudaSuccess)
	{
		fprintf_s(g_pFile, "CUDA ERROR : cudaMalloc failed at line %d\n",__LINE__);
		return INIT_CUDA_MALLOC_FAILED;
	}
	cuErr = cudaMalloc((void**)&dev_outSrc,4*DIM_W*DIM_H);
	if(cuErr!=cudaSuccess)
	{
		fprintf_s(g_pFile, "CUDA ERROR : cudaMalloc failed at line %d\n",__LINE__);
		return INIT_CUDA_MALLOC_FAILED;
	}
	cuErr=cudaMalloc((void**)&dev_constSrc,4*DIM_W*DIM_H);
	if(cuErr!=cudaSuccess)
	{
		fprintf_s(g_pFile, "CUDA ERROR : cudaMalloc failed at line %d\n",__LINE__);
		return INIT_CUDA_MALLOC_FAILED;
	...
	cuErr = cudaBindTexture(NULL,texConstSrc,dev_constSrc,4*DIM_W*DIM_H);
	if(cuErr!=cudaSuccess)
	{
	...
	cuErr = cudaBindTexture(NULL,texIn,dev_inSrc,4*DIM_W*DIM_H);
	if(cuErr!=cudaSuccess)
	{
		fprintf_s(g_pFile, "CUDA ERROR : cudaBindTexture failed at line %d\n",__LINE__);
		return CUDA_INIT_BIND_TEX;
	...
	cuErr = cudaBindTexture(NULL,texOut,dev_outSrc,4*DIM_W*DIM_H);
	if(cuErr!=cudaSuccess)
	{
		fprintf_s(g_pFile, "CUDA ERROR : cudaBindTexture failed at line %d\n",__LINE__);
		return CUDA_INIT_BIND_TEX;
	...
	float *temp = (float*)calloc(4*DIM_W*DIM_H,sizeof(char));
	for(int i=0; i < 4*DIM_W*DIM_H; i++)
	{
		temp[i] = 0;
		int x = i % DIM_W;
		int y = i / DIM_H;
	...
	temp[DIM_W*100+100] = (MAX_TEMP + MIN_TEMP)/2;
	temp[DIM_W*700+100] = MIN_TEMP;
	temp[DIM_W*300+300] = MIN_TEMP;
	temp[DIM_W*200+700] = MIN_TEMP;
	for(int y = 800;y<DIM_H;y++)
	{
		for(int x = 800;x<DIM_W;x++)
		{
			temp[x+y*DIM_W] = MAX_TEMP;
		}
	}

	cuErr = cudaMemcpy(dev_inSrc,temp,4*DIM_W*DIM_H,cudaMemcpyHostToDevice);
	if(cuErr != cudaSuccess)
	{
...
int Update(void)
{
	...
	dim3 threads(16,16);
	dim3 blocks(ceil(DIM_W/threads.x),ceil(DIM_H/threads.y));
	...
	float_to_color<<<blocks,threads>>>(writeSurface,dev_inSrc,dim3(DIM_W,DIM_H));
