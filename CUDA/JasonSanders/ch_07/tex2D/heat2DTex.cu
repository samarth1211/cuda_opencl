...
void swapFunc(float *ip1,float *ip2)
{
	float *temp = ip1;
	ip1 = ip2;
	ip2 = temp;
...
/**
*	Constant Kernel
* reads from our buffers which hold the position of heaters
*/
...
int Initialize(void)
{
	...
	// Bind Texture with cuda Memory
	cudaChannelFormatDesc desc = cudaCreateChannelDesc<float>();

	cuErr = cudaBindTexture2D(NULL,texConstSrc,dev_constSrc,desc,DIM,DIM,sizeof(float)*DIM);
	if(cuErr!=cudaSuccess)
	{
		fprintf_s(g_pFile, "CUDA ERROR : cudaBindTexture2D failed at line %d\n",__LINE__);
		return CUDA_INIT_BIND_TEX;
	}

	cuErr = cudaBindTexture2D(NULL,texIn,dev_inSrc,desc,DIM,DIM,sizeof(float)*DIM);
	if(cuErr!=cudaSuccess)
	{
	...
	cuErr = cudaBindTexture2D(NULL,texOut,dev_outSrc,desc,DIM,DIM,sizeof(float)*DIM);
	if(cuErr!=cudaSuccess)
	{
		fprintf_s(g_pFile, "CUDA ERROR : cudaBindTexture failed at line %d\n",__LINE__);
		return CUDA_INIT_BIND_TEX;
	...
	cuErr = cudaMemcpy(dev_constSrc,temp,4*DIM*DIM,cudaMemcpyHostToDevice);
	if(cuErr != cudaSuccess)
	{
...
int UnInitialize(void)
{
	...
	if (status != cudaSuccess)
	{
		fprintf_s(g_pFile,"IN UnInitialize()  cudaEventDestroy failed...!! \n");
	}
	...
	status = cudaUnbindTexture(texConstSrc);
	if(status!=cudaSuccess)
	{
		fprintf_s(g_pFile, "CUDA ERROR : cudaUnbindTexture failed at line %d\n",__LINE__);
	}

	status = cudaUnbindTexture(texIn);
	if(status!=cudaSuccess)
	{
		fprintf_s(g_pFile, "CUDA ERROR : cudaUnbindTexture failed at line %d\n",__LINE__);
	}

	status = cudaUnbindTexture(texOut);
	if(status!=cudaSuccess)
	{
		fprintf_s(g_pFile, "CUDA ERROR : cudaUnbindTexture failed at line %d\n",__LINE__);
	}
	...
	if (status != cudaSuccess)
	{
		fprintf_s(g_pFile, "IN UnInitialize() cudaDestroySurfaceObject failed...!! \n");
	}
