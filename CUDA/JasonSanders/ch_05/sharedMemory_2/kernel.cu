...
__global__ void sharedMem_kernel(cudaSurfaceObject_t target, dim3 texDim, float time)
{
	int x = blockIdx.x * blockDim.x + threadIdx.x;
	int y = blockIdx.y * blockDim.y + threadIdx.y;
	//int offset = x + y * blockDim.x * gridDim.x;
	...
	const float period = 155;

	if (x < texDim.x && y < texDim.y)
	{
		
		shared[threadIdx.x][threadIdx.y] = 255 * (time * sinf(x * 2.0f * PI /period)+1.0f) * (time * sinf(y * 2.0f *PI /period)+1.0f) / 5.0f;
		__syncthreads();
		uchar4 data = make_uchar4(0, shared[threadIdx.x][threadIdx.y], 0,255);
...
int Update(void)
{
	...
	dim3 thread(16,16);
	dim3 block(DIM/ thread.x, DIM / thread.y);
	sharedMem_kernel<< <block, thread >> >(writeSurface, dim3(DIM, DIM),g_fanimate);
